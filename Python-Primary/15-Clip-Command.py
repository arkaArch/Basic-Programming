...
get_snap_drive = "lsblk | grep 'snap' | head -1 | awk '{print $2}' | sed -e 's/└─//' -e 's/arch-//' -e 's/--/-/g'"
snap_drive = subprocess.getoutput(get_snap_drive)

COMMANDS = {
    'create': f"sudo lvcreate -L 5GB -s -n root-snap-{formatted_date} /dev/mapper/arch-root",
    'remove': f"sudo lvremove /dev/arch/{snap_drive}",
    'merge': f"sudo lvconvert --merge /dev/arch/{snap_drive}"
}
